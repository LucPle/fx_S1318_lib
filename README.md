...
| FX_SYSTEM | Sign Bit | Integer Bits | Fractional Bits |
|:---------:|:--------:|:------------:|:---------------:|
|  fx_S1318 |     1    |      13      |        18       |

* MSB부터 1bit가 부호를 나타낸다. 이때 Sign bit가 0이면 양수를, Sign bit가 1이면 음수를 의미한다.
* 그 다음 13bits는 정수 자리를 나타낸다.
* 남은 18bits는 소수점 아래 부분을 나타낸다.
...
* 자세한 구현 방식은 Part5 구현 분석에서 추가로 설명한다.

#### perf.c: 곱셈과 나눗셈 함수의 성능을 측정
* 8가지 함수를 1억번씩 실행하여 함수의 실행 시간을 측정한다.
* 이때, 32비트와 64비트 모두 측정한다. (컴파일러 옵션을 이용)
* 빠른 측정 결과 확인을 위해 각각 32bits_test.txt와 64bits_test.txt파일에 미리 저장했다.
* 자세한 구현 방식은 Part4 정밀도 분석에서 추가로 설명한다.
...
* 곱셈: 기본적으로 fx_S1318의 표현 범위인 -8192~8191.999996를 벗어나면 비정상적인 결과를 리턴한다.
    ...
    2. mul_bal(): mul_fast보다 안전하게 곱하는 함수이다.
        ...
        * 따라서 두 값중 하나라도 2<sup>-9</sup>보다 작은 값을 넣으면 무조건 0을 리턴한다. <br>
        ![image.png](./image/image3.png)
    3. mul_fast(): mul_bal보다 빠르지만, 장단점이 존재한다.
        * 두 값을 곱한 이후 오른쪽 쉬프트를 실시하는데, 2<sup>-18</sup>보다 작을 경우 0을 리턴한다.
        * 따라서 mul_bal에 비해 작은 값의 연산 범위가 크다.
        * 그러나 mul_bal에 비해 더 빠르게 오버플로가 발생한다. (먼저 곱셈을 진행하기 때문이다.) <br>
        ![image.png](./image/image4.png)
    4. mul_long(): mul_float와 유사하게 안전한 곱셈을 지원한다.
* mul_bal은 mul_fast에 비해 작은 값 연산에 취약하다. (0.001949을 제대로 계산하지 못한다.)
* mul_fast는 mul_bal보다 큰 값 연산에 취약하다.
* 안정적인 연산을 위해서는 mul_float와 mul_long을 사용해야 한다. <br>
![image.png](./image/image5.png)
* 나눗셈: 0으로 나누는 경우 0을 리턴하고, 그 외의 경우 정상적으로 수행한다.
    1. div_float(): 최대한 안전하게 나누는 함수이다.
    2. div_prev(): 먼저 왼쪽쉬프트로 값을 증가시킨 이후 나누는 함수이다.
        * 따라서 피제수(a/b에서 a)가 2<sup>-4</sup>이하인 경우 0을 리턴한다. <br>
        ![image.png](./image/image6.png)
    3. div_next(): 먼저 나눈 이후 왼쪽쉬프트로 값을 증가시키는 함수이다.
        * 왼쪽쉬프트를 마지막에 수행하므로 소수점을 제대로 표현할 수 없다. <br>
        ![image.png](./image/image7.png)
    4. div_long(): mul_float와 유사하게 안전한 나눗셈을 지원한다.
* div_prev는 피제수가 2<sup>-4</sup>보다 큰 경우 사용해야 한다.
* div_next는 소수점을 제대로 표현하지 못하므로 소수점이 필요 없는 경우 사용해야 한다.
* 안정적인 연산을 위해서는 div_float와 div_long을 사용해야 한다. <br>
![image.png](./image/image8.png)

## 5. 구현 분석
    * 삼각함수의 구현 방식을 설명한다.
    * 0° <= deg && deg <= 360°의 정수 deg만 받는다.
    * sinTable은 0°부터 90°까지의 sin(deg)값을 갖는 배열이다.
1. sinDeg(): sinTable을 이용하여 네 가지 경우에 따라 sin값을 계산한다.
	1. 0° <= deg && deg <= 90°인 경우: toFloat(sinTable\[deg])
    2. 90° < deg && deg <= 180°인 경우: toFloat(sinTable\[180 - deg])
    3. 180° < deg && deg <= 270°인 경우: (-1) * toFloat(sinTable\[deg - 180])
    4. 270° < deg && deg <= 360°인 경우: (-1) * toFloat(sinTable\[360 - deg])
2. cosDeg(): sinDeg()를 이용하여 두 가지 경우에 따라 cos값을 계산한다.
    1. 0° <= deg && deg <= 270°인 경우: sinDeg(deg + 90)
    2. 270° < deg && deg <= 360°인 경우: sinDeg(deg - 270)
3. tanDeg(): sinDeg()와 cosDeg()를 이용하여 tan값을 계산한다.
...
* make perf_32: 32bits 환경에서 곱셈과 나눗셈의 퍼포먼스 측정을 할 때 사용한다.
    * 실행파일을 실행한 이후, gprof "FILE" gmon.out을 이용하여 test page를 볼 수 있다.
